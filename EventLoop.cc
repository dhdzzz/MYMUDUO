...
{    
    ...
    if (t_loopInThisThread) {
        LOG_FATAL("Another EventLoop %p existed in this thread %d \n", t_loopInThisThread, threadId_);
    } else {
        t_loopInThisThread = this;
    }

    // 设置wakeupofd的事件类型以及发生事件后的回调操作
...
void EventLoop::loop() {
    ...
    while (!quit_) {
        ...
        // 监听梁磊磊fd  一种是client的fd, 一种是wakeupfd
...
void EventLoop::quit() {
    quit_ = true;
    // 如果是在其它线程中, 调用的quit 在一个subLoop(worker)中，调用了mainLoop(IO)的quit
    if (!isInLoopThread()) { 
        wakeup();
    }
}
